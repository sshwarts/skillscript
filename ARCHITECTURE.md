...
One-page map of the `skillscript-runtime` codebase. The *narrow core* (parser + compile + runtime + lint + connectors/) stays under a nudged LOC ceiling — currently **12700** across fewer than 23 files, enforced by `scripts/loc-ceiling.mjs` (the full nudge history, one line per feature-driven bump, lives in that file's header comment). Auxiliary surface (CLI, dashboard, MCP server, scheduler, observability, config loaders) is reported but doesn't gate the build. Tests count separately.
...
src/
  ...
  supervisor.ts         — autonomous-fire failure supervision: scheduler-tick trace sweep routes non-clean cron/event runs to a handler skill
...
| File | Responsibility |
| --- | --- |
| `cli.ts` | `skillfile` CLI. Commands: `init`, `execute`, `compile`, `audit`, `lint`, `list`, `fires`, `diagram`, `sign`, `verify`, `approve`, `reapprove`, `delete`, `replay`, `health`, `serve`, `dashboard`. Per-command `--help`; version from `version.ts`. |
...
| `supervisor.ts` | Autonomous-fire failure supervision. On each scheduler tick, sweeps the durable trace for non-clean cron/event runs (non-empty `errors`, `deadline_exceeded`, or uncertain effects) since the last sweep and routes each to an operator-configured handler skill (`SKILLSCRIPT_SUPERVISOR_SKILL`). Detection is built-in; notification is the handler's policy. Keyed on completion time, so a long unbounded run is caught whenever it finishes. |
...
| `observed-shape.ts` | Records the shape (keys + types, never values) of what a `$ conn.tool` dispatch returns under approval — feeds connector discovery (`skill_preflight` + `runtime_capabilities({tool})`). |
...
tests/                  — vitest specs (~2,340 tests across 181 files, incl. the example-connector spec
                          and the examples-corpus guard)
.github/workflows/      — ci.yml (typecheck + tests on push/PR to main, matrix: ubuntu/macos/windows)
                          + release.yml (fires on tag push)

...
**Where this came from.** [Issue #3](https://github.com/sshwarts/skillscript/issues/3) fixed a `$set` corruption in 0.39.4. But the reporter's sweep ran **12+ days over 13 real records reporting "0 processed" with zero errors logged** — and the corruption explains why no record was processed, not the silence. The silence was this: every branch of his loop body sat behind an `if` referencing a field that could not be resolved, and an unresolved operand compared false. **"The field is missing" and "the field says no" were the same answer.** That is independent of the `$set` bug and survived fixing it — any typo, renamed field, or changed response shape reproduces it.

- **All condition forms now raise:** bare truthy, `==`, `!=`, `<`/`>`/`<=`/`>=`, ref-to-ref comparisons, and the `in` left-hand side. (`in`'s right-hand side already raised.) Each operand is checked **individually** — a compound condition has as many failure sites as it has operands.
- **The error names the polarity, because absence did not only suppress work — it could invent it.** `if not ${X.absent}:` **fired** its branch, and `if ${X.absent} != "y":` did too, since `"" != "y"`. In the corpus this shape guarded a physical-hardware command. The error now states whether absence would have FIRED or SKIPPED the branch.
- **`|fallback:` is the escape hatch, and it now works in all three contexts.** It fired in comparisons but was **silently ignored in bare-truthy position** — so `if not ${X.maybe|fallback:"true"}:` behaved exactly as if the filter were absent. That is fixed here, in the same release, because it is the migration path this change depends on.
- **A non-`fallback` filter does not suppress the raise.** `${X.absent|trim}` still raises; only an explicit `|fallback:` declares absence expected.
- **New tier-2 lint `unguarded-dotted-ref-in-condition`.** A dotted reference used as a condition operand with no `|fallback:`. Deliberately a warning, not an error: the rule detects a *shape* and cannot read a response contract, so it cannot tell a correct site from an incorrect one — and a check that cannot distinguish right from wrong should not block a write. References inside a filter *argument* are excluded (those already raised).

**Inside a `foreach`, the blast radius is the whole loop.** `foreach` has no per-iteration error boundary, so a raise in the loop body aborts the entire target — every record after the bad one in iteration order gets zero processing that run. Previously such a condition silently skipped the bad record and the batch continued. **This is the one place where adding `|fallback:` is about per-record isolation rather than suppression**, and it is a legitimate use; see [UPGRADING.md §8](UPGRADING.md). Raised by an adopter running a daily sweep over externally-sourced records — the shape the rest of this entry does not cover.

**No in-language catch.** A raising condition aborts the target. `else:` cannot catch it (the condition itself raised, so no branch is selected), `(fallback: …)` is an op trailer rather than a condition guard, and `# OnError:` was removed in 0.39.0. This is deliberate — the failure it replaces was invisible, and a catchable version would invite re-suppressing it.

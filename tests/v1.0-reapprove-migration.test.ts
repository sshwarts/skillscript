...
// Store a genuinely v3-Approved skill: the unsecured store auto-restamps every
// write to v1, so the v3 signature only survives when secured mode + the public
// key are armed for the write (the store then honors the matching signature).
// Toggle back off after — the spawned CLI is a separate process either way.
 ...
 *
 * Secured mode rejects pre-secured-mode v1 hash-stamps (they can't distinguish
 * a human approval from an agent self-approval). The default is force-re-approve
 * — existing Approved skills are NOT grandfathered. This command sweeps the
...
describe("skillfile reapprove — force-re-approve migration", () => {
  it("dry-run reports v1-stamped Approved skills as the migration set; v3 ones as already valid", async () => {
    const { home, skillsDir, pubFile, keyFile } = makeHome();
    const store = new FilesystemSkillStore(skillsDir);
    // legacy: Approved under a v1 hash-stamp (the pre-secured-mode artifact)
    await store.store("legacy-a", stampApprovalToken(body("legacy-a"), "v1"), { status: "Approved" });
    await store.store("legacy-b", stampApprovalToken(body("legacy-b"), "v1"), { status: "Approved" });

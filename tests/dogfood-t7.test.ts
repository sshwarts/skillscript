...
describe("T7 — package.json polish", () => {
  it("1. version is 0.2.8 (T7 + 0.2.1-0.2.7 polish + 0.2.8 help + execute_skill)", () => {
    expect(PACKAGE_JSON["version"]).toBe("0.2.8");
  });
...
describe("T7 — CLI --help surface", () => {
  ...
  it("11. version flag reports 0.2.8", () => {
    const out = execSync(`${CLI} --version`, { encoding: "utf8" });
    expect(out.trim()).toBe("0.2.8");
  });
});

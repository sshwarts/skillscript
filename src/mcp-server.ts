...
export class McpServer {
  ...
  private registerBuiltinTools(): void {
    ...
    this.registerTool({
      name: "skill_write",
      description: "Write a skill body into the configured SkillStore. Tier-1 lint runs at write time (SkillStore contract) and throws on rejection — run lint_skill / compile_skill first to iterate cleanly. The `# Status:` header is honored: `# Status: Approved` lands Approved in unsecured mode; in secured mode an unsigned body is forced to Draft (a write can't grant approval). NOTE: a returned status of Draft records approval state — it is NOT by itself a block on every execution path. On this runtime a Draft is rejected before it runs via `execute_skill({name})` and cron/event triggers (the approval gate), but an ad-hoc `execute_skill({source})` body runs regardless of its `# Status:` (effects then gated by secured mode only). Enforcement reads the `# Status:` header IN THE STORED BODY, not this returned metadata — so a custom SkillStore that records a Draft downgrade in metadata WITHOUT rewriting the body header leaves the body gate-passing; the bundled stores rewrite the body to keep the two in sync. Promote later via skill_status, or the dashboard / `skillfile approve` in secured mode. Returns version + content_hash. Write operation.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    this.registerTool({
      name: "execute_skill",
      description: "Execute a skill end-to-end against the runtime's wired connectors. The two modes have DIFFERENT approval semantics — read this before relying on it as a safety control. (1) `name` — execute a stored skill; the runtime fetches from SkillStore and the v0.9.0 approval gate fires BEFORE the body runs, rejecting a Draft / Disabled / tampered body in BOTH secured and unsecured mode (a stored Draft cannot run by name). (2) `source` — ad-hoc inline execution; the supplied body runs in memory and is discarded. It is NOT status-gated: the body runs regardless of its `# Status:`. Effect enforcement is then secured-mode-dependent — in UNSECURED mode a `source` body's effectful ops FIRE FOR REAL (it can send mail, write data, run shell), Draft or not; in SECURED mode those effects are refused unless the body itself carries a valid approval signature (an operator approved exactly that body). So: `name` = gated (Draft blocked); `source` in unsecured mode = fully live regardless of status. Use `name` for production / autonomous dispatch; treat an unapproved `source` body in unsecured mode as live. Returns {skill_name, final_vars, transcript, outputs, errors, target_order}. `mechanical: true` previews dispatch without firing $/~/@/?? ops. Recursion-depth-guarded (default 10). Write operation. v0.15.2 — `skill_name` is accepted as a silent back-compat alias for `name`.",
      inputSchema: {
        type: "object",
        properties: {
          ...
          source: { type: "string", description: "Ad-hoc inline skill body. Runs in memory; never persisted. NOT status-gated — the body runs regardless of its `# Status:`, and in UNSECURED mode its effectful ops fire for real (secured mode refuses effects unless the body carries a valid approval signature). Exactly one of `name` / `source` is required." },
    ...
    this.registerTool({
      name: "help",
      description: "Cold-agent language discovery. `help()` returns a ~500-token quickstart. `help({topic})` returns a deeper section. Topics: ops / frontmatter / examples / composition / error-handling / connectors / lint-codes. Read-only.",
      inputSchema: {
        type: "object",
        properties: {
          topic: {
            type: "string",
            enum: ["ops", "frontmatter", "examples", "composition", "error-handling", "connectors", "lint-codes"],

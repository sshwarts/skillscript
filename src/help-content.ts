...
- **Bounding — two levers, pick by intent:** For a **partial-tolerant** run (a gather where one slow leg shouldn't sink the rest), bound **each leg** with a per-op \`timeout=N\` (seconds) + a \`(fallback: ...)\`. A timed-out leg throws a **catchable** error, so the fallback binds and the run continues — the everyday tool, and it works on any \`$\` connector op (the runtime enforces \`timeout=\` regardless of whether the remote tool's schema lists it). \`# Timeout: N\` sets a per-op default for the whole skill. \`# Deadline: N\` is the **other** lever: a whole-run wall-clock cap that terminates **uncatchably** (op \`(fallback:)\`/\`else:\` can't recover it) and aborts **every** leg including healthy ones — reserve it for a hard total ceiling, *not* the per-leg bound for a gather. Lint (\`unbounded-no-deadline\`) nudges only when a leg has no bound at any level. See \`help({topic: "frontmatter"})\` and \`help({topic: "error-handling"})\`.
...
(The former \`# OnError: <skill>\` header was **removed** — it was never wired to runtime error handling, so a skill relying on it had no recovery at all. A skill carrying it now fails to compile with a pointer here. Use \`else:\` for target-level recovery, or \`(fallback: "…")\` to degrade a single op.)
...
**Return shapes are tool-native.** External tools return whatever shape they natively return; the language doesn't normalize. Some tools return a **structured envelope** (e.g., \`$ youtrack.search_issues -> R\` → \`R.issuesPage[]\`); some return **pre-formatted text** (e.g., \`$ ddg.search -> R\` → a search-results text blob). Inspect the shape before writing \`\${R.field}\` or \`foreach IT in \${R.results}\`: **\`runtime_capabilities({tool: "<connector>.<tool>"})\` reports the tool's \`observed_output_shape\`** — the actual return structure recorded from prior runs (not just the input schema), so you can validate field access pre-commit without running the op yourself. Mistaking a text-blob return for a structured envelope is the canonical cold-author trap.
...
- \`unbounded-no-deadline\` — a \`$\` / \`shell\` / \`notify\` leg has no time bound at any level (no per-op \`timeout=\`, no \`# Timeout:\`, no \`# Deadline:\`), so it can hang. Fix: bound the leg with a per-op \`timeout=N\` + a \`(fallback: ...)\` — catchable, so the rest of the run continues (a \`$\` op carrying its own \`timeout=\` doesn't trip this). \`# Timeout: N\` bounds every op in the skill. \`# Deadline: N\` is a *different* tool — a whole-run **uncatchable** cap that aborts healthy legs too; reserve it for a hard total ceiling, not a partial-tolerant gather. Advisory only.

...
export class SqliteSkillStore implements SkillStore {
  ...
  async delete(name: string): Promise<void> {
    ...
    // reverse-dependency warning, so by the time we reach the substrate the
    // operator has accepted the loss. The name frees up immediately for reuse;
    // no orphan history is left behind.

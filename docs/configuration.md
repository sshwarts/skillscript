...
| `SKILLSCRIPT_MAX_DEADLINE_SECONDS=120` | Hard ceiling on the wall-clock time of **every** run — enforced even when a skill declares no `# Deadline:`; a skill's own deadline can only tighten it. The guard against an agent author evading the time bound. **Unset = no ceiling.** See [adopter-playbook](adopter-playbook.md) § "Bounding runs — deadlines & cancellation". | `bootstrap()` opt > env > config > default (no ceiling) |
| `SKILLSCRIPT_SUPERVISOR_SKILL=<skill>` | Approved handler skill the scheduler's trace-sweeper routes **non-clean autonomous fires** (errored / deadline-exceeded / uncertain-effects) to. Setting this turns autonomous-fire failure supervision ON. **Requires scheduler tracing to be on** — the runtime hard-refuses at boot otherwise (the sweeper reads the durable trace). Unset = off. See [adopter-playbook](adopter-playbook.md) § "Autonomous-fire failure supervision". | `bootstrap()` opt > env > config > default off |
| `SKILLSCRIPT_SUPERVISOR_AGENT=<agent-id>` | *Optional.* Agent id passed to the handler as `${SUPERVISOR_AGENT}` (for an agent-routed handler; unneeded for an email/webhook handler). | `bootstrap()` opt > env > config > default unset |

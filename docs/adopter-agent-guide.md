...
**When an autonomous skill fails, nobody's watching the return value.** If the operator has configured failure supervision (`SKILLSCRIPT_SUPERVISOR_SKILL`), the runtime pushes non-clean autonomous runs — errored, deadline-cut, or "a mutation may have landed" — to a **handler skill**, so failures surface instead of vanishing. That handler is itself a normal skill (an agent may be asked to author or maintain it); it receives the failure as vars and decides how to notify (agent, email, webhook). If you own autonomous skills, know whether supervision is on — it's the difference between a 3 a.m. failure paging someone and disappearing into the trace.

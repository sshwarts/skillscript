...
**`delete()` is soft in the bundled impls — recoverable, name-reclaiming.** The contract signature (`delete(name): Promise<void>`) is unchanged, but the bundled stores tombstone rather than erase: `FilesystemSkillStore` moves the files to a `.trash/` subdir, `SqliteSkillStore` stamps a `deleted_at` column. A deleted skill is excluded from `query()`/`load()`/`metadata()`/`versions()` (so it disappears from every listing, including an agent's `skill_list`), the name frees up for a fresh `store()` (which supersedes the tombstone with clean history), and the source + history are retained for recovery. Delete is **operator-only** (CLI `skillfile delete` / the dashboard) — there is no agent/MCP delete surface. Your impl *may* hard-delete instead; the runtime only requires "remove from normal views," and recovery semantics are your store's concern.
